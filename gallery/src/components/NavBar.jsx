...
const [vhInPixels, setVhInPixels] = useState(100); 
    ...
    const START_SCROLL = vhInPixels * 0.4; // Starts fading at 50% of viewport height
    const END_SCROLL = vhInPixels * 0.9;   // Fully visible at 100% of viewport height
    ...
    return (
        <motion.nav 
            className="navbar" 
            style={{ 
                // 🔑 Apply the scroll-dependent opacity
                opacity: navOpacity,
                // The background color opacity already works fine with the existing transform
                backgroundColor: useTransform(navBgOpacity, (o) => `rgba(197, 195, 195, ${o})`) 
            }}
        >
            {/* Logo / Home Link (Uses Native Scroll to Top) */}
